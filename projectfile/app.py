...
def confirm_booking(show_id):
    ...
    discount_rate = update_user_discounts(user_id)

    # Calculate total cost and amount saved
    total_cost = float(ticket_price * seats) * (1 - discount_rate / 100)
    amount_saved = float(ticket_price * seats) * (discount_rate / 100)
    ...
    # Pass the discount and amount saved to the confirmation page
    return redirect(url_for('booking_confirmation', booking_id=booking_id, amount_saved=amount_saved))



@app.route('/booking_confirmation/<int:booking_id>')
def booking_confirmation(booking_id):
    amount_saved = request.args.get('amount_saved', 0)  # Retrieve the amount saved from query params
    with db.cursor() as cursor:
        cursor.execute("""
    ...
    return render_template(
        ...
        booking=booking,
        amount_saved=amount_saved  # Pass the amount saved to the template
    )
...
def update_user_discounts(user_id):
    with db.cursor() as cursor:
        ...
        # Calculate discount
        discount_rate = min(20, watched_count * 2)  # 2% per watched show, max 20%
        ...
        return discount_rate  # Return the new discount rate
